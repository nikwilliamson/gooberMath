...
export const Nodes: StoryObj<typeof QuestNode> = {
  render: () => (
    <div className="nodes">
      <QuestNode {...nodeArgs} index={0} quest={quest('add-1')} status="cleared" marker="mastered" learned={12} required={12} mastered perfect bestSniper={4120}>
        <FactGrid keys={questKeys(quest('add-1'))} stats={statsAt(questKeys(quest('add-1')), 'automatic')} />
      </QuestNode>
      <QuestNode {...nodeArgs} index={1} quest={quest('add-2')} status="cleared" marker="cleared" learned={7} required={8} bestSniper={3480}>
        <FactGrid keys={questKeys(quest('add-2'))} stats={statsAt(questKeys(quest('add-2')), 'known')} />
      </QuestNode>
      <QuestNode {...nodeArgs} index={2} quest={quest('add-3')} status="open" marker="current" bestSniperVoice={1420}>
        <FactGrid keys={questKeys(quest('add-3'))} stats={statsAt(questKeys(quest('add-3')), 'mixed')} />
      </QuestNode>
      <QuestNode {...nodeArgs} index={3} quest={quest('add-4')} status="open" marker="open" learned={0} bestSniper={0}>
        <FactGrid keys={questKeys(quest('add-4'))} stats={{}} />
      </QuestNode>
      <QuestNode {...nodeArgs} index={5} quest={quest('add-boss')} status="locked" marker="locked" learned={0} bestSniper={0}>
...
export const BarCleared: StoryObj<typeof QuestBar> = { ...Bar, args: { ...barArgs, cleared: true, mastered: true, learned: 10 } }
